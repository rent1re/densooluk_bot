...
likes = set()  # Множество user_id, кто поставил лайк
dislikes = set()  # Множество user_id, кто поставил дизлайк
...
def create_keyboard():
    ...
    # Кнопка "Лайк" с количеством лайков
    like_button = InlineKeyboardButton(f"👍 {len(likes)}", callback_data='like')
    # Кнопка "Дизлайк" с количеством дизлайков
    dislike_button = InlineKeyboardButton(f"👎 {len(dislikes)}", callback_data='dislike')
...
def callback_query(call):
    ...
    user_id = call.from_user.id  # Получаем user_id пользователя

    # Проверяем, какую кнопку нажали
    if call.data == 'like':
        if user_id not in likes:  # Если пользователь ещё не ставил лайк
            likes.add(user_id)  # Добавляем user_id в множество лайков
            dislikes.discard(user_id)  # Убираем из дизлайков, если есть
        else:
            likes.remove(user_id)  # Если уже был лайк, убираем его
    elif call.data == 'dislike':
        if user_id not in dislikes:  # Если пользователь ещё не ставил дизлайк
            dislikes.add(user_id)  # Добавляем user_id в множество дизлайков
            likes.discard(user_id)  # Убираем из лайков, если есть
        else:
            dislikes.remove(user_id)  # Если уже был дизлайк, убираем его

    # Создаём обновлённую клавиатуру
    new_keyboard = create_keyboard()
    # Проверяем, отличается ли новая клавиатура от текущей
    if call.message.reply_markup != new_keyboard:
